...
if __name__ == '__main__':
    ...
    denoiser_name = 'bfdncnn'
    PnP_iter = 200
    n = 64 * 64
    m_list = [int(n / 4), int(n / 2), int(3 * n / 4)]
    mu = 0.5  # gradient update step size
    sigma = 10  # highest observed noise level in training of denoiser
    ...
    bfdncnn_denoiser = load_model(f'pretrained/dncnn/bias_free/0-{int(sigma)}.pt')
    bfdncnn_denoiser.eval()
    ...
    for j, m in enumerate(m_list):
        ...
        for p in (pbar := tqdm(range(PnP_iter))):
            ...
            x_hat = bfdncnn_denoiser(torch.Tensor(s.reshape(64, 64))[None, None, :, :]).squeeze().detach().numpy().reshape(n, 1)
